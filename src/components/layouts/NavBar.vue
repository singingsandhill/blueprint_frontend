  ...
  <nav class="bg-[#002842] text-white shadow-lg rounded-xl z-50 relative">
    <div class="container mx-auto px-6 flex justify-between items-center h-16">
      ...
      <div class="flex items-center space-x-8">
        ...
        <ul class="hidden lg:flex space-x-12 md:space-x-4 text-base md:text-sm py-2">
          <li class="list-none py-2">
            <router-link to="/policy" class="text-white hover:text-gray-200 transition-colors duration-200 py-2">
              정책
            </router-link>
          </li>
          <li class="list-none py-2">
            <router-link to="/subscription" class="text-white hover:text-gray-200 transition-colors duration-200 py-2">
              청약
            </router-link>
          </li>
          <li class="list-none py-2">
            <router-link to="/realestate" class="text-white hover:text-gray-200 transition-colors duration-200 py-2">
              부동산
            </router-link>
          </li>
          <li class="list-none py-2">
            <router-link to="/finance" class="text-white hover:text-gray-200 transition-colors duration-200 py-2">
              금융
            </router-link>
          </li>
        </ul>
      ...
      <!-- 우측: 계정 메뉴 -->
      <div class="hidden lg:flex space-x-8 md:space-x-4 items-center text-base md:text-sm py-2">
        <li class="list-none py-2">
          <router-link to="/myservice" class="text-white hover:text-gray-200 transition-colors duration-200 py-4">
            나만의 서비스
          </router-link>
        </li>
        <template v-if="isLogin">
          <router-link to="/member/mypage" class="text-white hover:text-gray-200 py-4">
            {{ memberName }}
          </router-link>
          <router-link to="/" @click.prevent="handleLogout" class="text-white hover:text-gray-200 py-4">
            로그아웃
          </router-link>
        </template>
        <template v-else>
          <router-link to="/member/login" class="text-white hover:text-gray-200 py-4">
            로그인
          </router-link>
          <router-link to="/member/register" class="text-white hover:text-gray-200 py-4">
            회원가입
          </router-link>
        </template>
      </div>
